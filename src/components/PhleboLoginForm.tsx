...
export function PhleboLoginForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/phlebo/login", {
        method: "POST",
  ...
  return (
    <form onSubmit={submit} className="mx-auto mt-16 max-w-sm space-y-4 rounded-xl border border-border bg-surface p-6">
      <div>
        ...
        <h1 className="font-display text-lg font-semibold text-ink">Phlebotomist login</h1>
      </div>
